...
describe('Empirical Challenge: Milestone M1 Website Modernization', () => {
  ...
  describe('4. Direct Release Download Integration & Fallback', () => {
    it('declares exact release URLs matching v2.5.3 binary specification', async () => {
      const { ZENDEV_RELEASE_CONFIG } = await import('../website/src/lib/downloadHelper');

      expect(ZENDEV_RELEASE_CONFIG.version).toBe('2.5.3');
      expect(ZENDEV_RELEASE_CONFIG.setupExe).toBe(
        'https://github.com/zerviatr/NexusHub/releases/download/v2.5.3/ZenDev-Setup-2.5.3.exe'
      );
      expect(ZENDEV_RELEASE_CONFIG.portableExe).toBe(
        'https://github.com/zerviatr/NexusHub/releases/download/v2.5.3/ZenDev-Portable-2.5.3.exe'
      );
