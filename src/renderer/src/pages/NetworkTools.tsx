...
import { nexusAPI, IpLookupResult, DnsQueryResult, PortScanResult, PingResult, DnsType } from '../lib/ipc'
import { useT } from '../lib/i18n'

type Tab = 'ip' | 'dns' | 'port' | 'ping'
      ...
      <div className="flex gap-1.5 mb-5 p-1 bg-nexus-card rounded-xl border border-nexus-border/50 overflow-x-auto">
        {TABS.map(({ id, label, icon: Icon }) => (
          <button
            key={id}
            id={`net-tab-${id}`}
            onClick={() => { setActiveTab(id); clearResults() }}
