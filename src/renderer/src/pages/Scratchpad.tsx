...
const DEFAULT_NOTE_CONTENT = `# NexusHub Scratchpad Pro

Geliştiriciler, sistem yöneticileri ve teknik ekipler için tasarlanmış yüksek performanslı çevrimdışı Markdown ve Kod stüdyosu.

## ⚡ Canlı Özellikler
- [x] Çoklu sekme ve bağımsız not yönetimi
- [x] Tıklanabilir canlı interaktif görev kutuları
- [x] Akıllı seçim sarma (Selection wrap) ve klavye kısayolları
- [x] Senkronize çift panel (Dual-scroll) kaydırma
- [x] Zengin tablo, kod bloğu ve alıntı render desteği
- [x] PDF yazdırma ve biçimlendirilmiş HTML kopyalama

### 🛠️ Örnek Tablo
| Protokol | Port | Açıklama | Güvenlik Seviyesi |
| :--- | :--- | :--- | :--- |
...
export default function Scratchpad() {
  ...
  const updateContent = (newContent: string) => {
    setNotes((prev) =>
      prev.map((n) => (n.id === activeNote.id ? { ...n, content: newContent, updatedAt: Date.now() } : n))
    )
  }
...
<pre style="white-space: pre-wrap; font-family: inherit; background: transparent; border: none; padding: 0;">${text.replace(/</g, '&lt;').replace(/>/g, '&gt;')}</pre>
  ...
  return (
    <BaseToolTemplate
      icon={FileText}
      title="Markdown Scratchpad Pro"
      description="Çoklu sekme yönetimi, canlı etkileşimli görevler, senkronize çift scroll, klavye kısayolları ve hazır teknik şablon kütüphanesi."
          ...
          <div className="flex items-center gap-1.5 overflow-x-auto pb-1 md:pb-0 scrollbar-none flex-1">
            {filteredNotes.map((note) => {
              ...
              return (
                ...
                >
                  <FileText className={`w-3.5 h-3.5 ${isActive ? 'text-nexus-cyan' : 'text-nexus-muted'}`} />
          ...
          <div className="flex items-center gap-2">
            <div className="relative">
              ...
              <input
                ...
                className="pl-8 pr-3 py-1 rounded-xl bg-nexus-surface/60 border border-white/10 text-xs text-white placeholder-nexus-muted focus:outline-none focus:border-nexus-cyan/40 w-36 sm:w-44"
              />
            </div>
          ...
          {(viewMode === 'edit' || viewMode === 'split') && (
            ...
            >
              ...
              <textarea
                ...
                placeholder="Markdown metninizi buraya yazın... (Ctrl+B, Ctrl+I, Tab desteklenir)"
                className="flex-1 w-full bg-transparent resize-none focus:outline-none font-mono text-xs text-white leading-relaxed placeholder-nexus-muted selection:bg-nexus-accent/30 overflow-y-auto"
              />
            </div>
          )}
