...
/**
 * Enterprise MFA Authenticator State Engine for ZenDev Admin Console
 */
class MfaAuthenticatorService {
  ...
  public enableMfa(secretBase32: string, verificationCode: string): { success: boolean; backupCodes?: string[]; error?: string } {
    ...
    // Generate 8 cryptographically secure emergency backup recovery codes
  ...
  public verifyChallenge(
    ...
    // Check if input is an emergency backup code
    ...
    if (this.hashedBackupCodes.has(inputHash)) {
      // Consume backup code immediately (single-use)
    ...
    // Verify TOTP token with +/- 1 time step tolerance (RFC 6238 drift window)
    ...
    for (const offset of [-1, 0, 1]) {
      ...
      if (this.usedCounters.has(checkCounter)) {
        continue // Replay protection: prevent reusing same code in same time window
      }
    ...
    if (matched) {
      ...
      if (this.failedAttempts >= 5) {
        this.lockedUntil = now + 5 * 60 * 1000 // 5 minute lock
      }
...
describe('MFA & TOTP Integration Authentication Flow', () => {
  ...
  describe('RFC 6238 TOTP Algorithm & Base32 Codec', () => {
    ...
    it('should produce identical codes within the same 30-second window', () => {
      const t1 = 1710000000000 // Fixed epoch ms
      const t2 = 1710000015000 // 15 seconds later (same 30s bucket)
    ...
    it('should produce different codes across different 30-second windows', () => {
      ...
      const t2 = 1710000035000 // 35 seconds later (next 30s bucket)
  ...
  describe('Two-Phase Authentication Challenge Lifecycle', () => {
    ...
    it('should prevent replay attacks by rejecting the exact same TOTP code within the same time window', () => {
      ...
      // Second challenge attempt with same code fails (replay protection)
    ...
    it('should successfully authenticate using an emergency backup recovery code and consume it', () => {
      ...
      // 1. First use of backup code succeeds
      ...
      // 2. Second attempt with the SAME backup code is rejected (burned)
      const { challengeToken: token2 } = mfaService.createChallenge()
